...
async function computeAllMetrics() {
  ...
  const [loopExposureRes, sEnaRatioRes] = await Promise.all([
    sources.fetchLoopExposure(usdeSupplyRes.ok ? usdeSupplyRes.value : null),
    sources.fetchSEnaStakingRatio(enaCirculatingSupply)
  ]);

  [enaMarketRes, ethPriceRes, usdeSupplyRes, susdeApyRes, tbillRes, fundingRes, convergeRes, loopExposureRes, sEnaRatioRes]
  ...
  const metrics = {
    ...
    reserveFundUsd: manual.reserveFund.totalUsd,
    reserveFund: manual.reserveFund,
